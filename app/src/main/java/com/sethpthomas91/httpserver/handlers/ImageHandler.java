...
public class ImageHandler implements Handler {
    private final String publicDir = System.getenv("PUBLIC_DIR");
    ...
    private HttpResponseWrapper handleBody(HttpRequestInterface httpRequest, HttpResponseWrapper httpResponse)throws IOException {
        Body body = new Body();
        String pubDir = System.getenv().get("PUBLIC_DIR");
        String filePath = httpRequest.getRequestLine().getUniformResourceIdentifier().split("/")[1];
        String path = pubDir + filePath;
        body.setBodyBytes(ByteArrayGenerator.convertFileToBytes(path));
